    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            ### PUT YOUR CODE HERE ###
            rate_ = (self.epsilon_start - self.epsilon_min) / (self.exploration_fraction * max_timestep)
            self.epsilon = self.epsilon_start - rate_ * timestep
            return max(self.epsilon_min, self.epsilon)

        def epsilon_exponential_decay(*args, **kwargs):               #Left to implement
            ### PUT YOUR CODE HERE ###
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        #Torch.size([4]) - tensor - ([int, int, int, int])
        

        if explore == True and np.random.rand() < self.epsilon:

            action = self.action_space.sample() # Eps-greedy: random action with pr = epsilon
        else:
            
            with torch.no_grad():
                q_values = self.critics_net(obs_tensor)             # Greedy: choose best possible action
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        next_q = self.critics_target.forward(next_states).detach()


        max_q, _ = next_q.max(dim=1, keepdim=True) # Compute max_q and target_q for each state
        target_q = rewards + (1 - dones) * self.gamma * max_q

        q_value = self.critics_net.forward(states).gather(1, actions) #Q-value for each s,a pair and loss
        q_loss = torch.nn.functional.mse_loss(q_value, target_q)

        self.critics_optim.zero_grad()
        q_loss.backward()
        self.critics_optim.step()

        self.update_counter += 1 # Update target network periodically
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        with torch.no_grad():
            ...
            action_probs = self.policy(obs_tensor)  # Get action probs
            action_distribution = torch.distributions.Categorical(action_probs) 
            action = action_distribution.sample().item()  # Sample action from the dist
        return action
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        # Reward-to-go is (G_t) for each time step
        G_t = []
        running_total = 0
        for r in reversed(rewards):
            running_total = r + self.gamma * running_total
            G_t.append(running_total)
        G_t = list(reversed(G_t)) 

        observations_tensor = torch.FloatTensor(observations)
        actions_tensor = torch.LongTensor(actions)
        G_t_tensor = torch.FloatTensor(G_t)

        action_probs = self.policy(observations_tensor)
        action_distribution = torch.distributions.Categorical(action_probs)
        log_probs = action_distribution.log_prob(actions_tensor)

        policy_loss = -torch.mean(log_probs * G_t_tensor)

        self.policy_optim.zero_grad() #Optiisation step
