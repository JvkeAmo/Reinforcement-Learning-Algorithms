...
class DDPG(Agent):
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        with torch.no_grad():
            action = self.actor.forward(tensor_obs)

        if explore == True:
            noise_s = self.noise.sample()
            action += noise_s
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        n_actions = self.actor_target(next_states)
        n_q_values = self.critic_target(torch.cat([next_states, n_actions], dim=1)) #estimated q-value of the next s, a pair from the target actor network
        target_q = rewards + self.gamma * n_q_values * (1 - dones)
        current_q = self.critic(torch.cat([states, actions], dim=1))

        q_loss = torch.nn.functional.mse_loss(current_q, target_q.detach())
        ...
        # Update 
        new_actions = self.actor(states) #Get new_actions from Network
        p_loss = -self.critic(torch.cat([states, new_actions], dim=1)).mean()
        
        self.policy_optim.zero_grad()
        p_loss.backward()
        self.policy_optim.step()

        self.actor_target.soft_update(self.actor, self.tau) # Performing soft_updates of Networks
        self.critic_target.soft_update(self.critic, self.tau) # Performing soft_updates of Networks
